...
public class FuriganaTextView extends AppCompatTextView {
    ...
    public FuriganaTextView(Context context, AttributeSet attrs, int style) {
    ...
    private void initialize(Context context, AttributeSet attrs) {
        ...
        mLineHeight = mTextSize + mFuriganaSize * 2 + mLineSpacing + mTextPadding;
    }
    ...
    private int measureWidth(int width) {
        ...
        while (!text.isEmpty()) {
            if (text.indexOf(LINE_BREAK) == 0 || text.indexOf("\n") == 0) {
                length += mNormalPaint.measureText(normal.toString());
                ...
                normal.append(text.substring(0, 1));
                text = text.substring(1);
            }
        }
    ...
    private float handleNewline(float width) {
        ...
        if (remainder > 0 && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1)
        ...
        void onDraw(Canvas canvas, float x, float y) {
            ...
            if (furiganaText == null) {
                ...
                if (normalWidth < furiganaWidth) {
                    ...
                    canvas.drawText(furiganaText, 0, furiganaText.length(), x, y - mTextSize - mTextPadding, furiganaPaint);
                } else {
                    float offsetX = x + offset;
                    for (int i = 0; i < furiganaText.length(); i++) {
                        canvas.drawText(furiganaText, i, i + 1, offsetX, y - mTextSize - mTextPadding, furiganaPaint);
